...
if __name__ == '__main__':
    ...
    parser.add_argument("--input_size", type=int, default=114)
    parser.add_argument("--layers", type=int, default=1)
    parser.add_argument("--dropout", type=int, default=0)
    parser.add_argument("--epoch", type=int, default=800)        # 训练周期
    parser.add_argument("--seq_len", type=int, default=50)       # 数据中的月数
    parser.add_argument("--house_size", type=int, default=2000)   # 每个月的房屋数目
    ...
    # 结果输出文件设置
    result_file_path = 'result_LSTM/'
    model_file_path = 'model_saved_LSTM/'
    other_file_path = 'result_LSTM/others/'
    ...
    train_index = range(9000)
    test_index = range(9000, 10000)
    ...
    model = LSTM_static(nfeat=input_dim, dropout=args.dropout, house_size=house_size).to(device)
    ...
    for i in range(train_epoch):
        ...
        out_price = model(features)  # LSTMs里的forward()函数，图卷积操作
